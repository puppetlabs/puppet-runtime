...
if platform.is_aix?
  ...
  pkg.environment "PATH", "$(cygpath -u #{settings[:gcc_bindir]}):$(cygpath -u #{settings[:tools_root]}/bin):$(cygpath -u #{settings[:tools_root]}/include):$(cygpath -u #{settings[:bindir]}):$(cygpath -u #{ruby_bindir}):$(cygpath -u #{settings[:includedir]}):${PATH}"
